...
const col = ['id', 'Firm', 'Material', 'quantity', 'unitPrice', 'totalPrice', 'createdAt',];

const PurchaseAccFirmInfoInfoTable = ({ rowData }: { rowData: any }) => {
    ...
    useEffect(() => {
        setInitialRecords(() => {
            return rowData?.filter((item: Purchase) => {
                return (
                    item.id.toString().includes(search.toLowerCase()) ||
                    item.Firm?.name.toLowerCase().includes(search.toLowerCase()) ||
                    ...
                    item.unitPrice.toString().toLowerCase().includes(search.toLowerCase()) ||
    ...
    const header = ['Id', 'Firm Name', 'Material Name', 'Quantity', 'Unit Price', 'Total Price', 'Date Created', 'Actions'];
    ...
    function handleDownloadExcel() {
        downloadExcel({
            ...
            tablePayload: {
                header,
                body: rowData.map((item: Purchase) => ({
                    id: item.id,
                    Firm: item.Firm.name,
    ...
    const exportTable = (type: any) => {
        let columns: any = col;
        let records = rowData;
        let filename = 'Purchase Account for ' + rowData[0].Firm.name;
        ...
        if (type === 'csv') {
            ...
            records.map((item: any) => {
                columns.map((d: any, index: any) => {
                    ...
                    if (d === 'Firm') val = item.Firm.name;
                    if (d === 'Material') val = item.Material.name;
                    result += val;
                });
                result += linedelimiter;
            });
            ...
            <div className="datatables">
                <DataTable
                    ...
                    columns={[
                        ...
                        {
                            accessor: 'Firm', title: 'Firm Name', sortable: true,
                            render: ({ Firm }) => <div>{Firm?.name}</div>
                        },
                        {
                            accessor: 'Purchase', title: 'Material Name', sortable: true,
                            render: ({ Material }) => <div>{Material?.name}</div>
                        },
                        { accessor: 'quantity', title: 'Quantity', sortable: true },
                        { accessor: 'unitPrice', sortable: true },
                        { accessor: 'totalPrice', sortable: true },
                        {
                            accessor: 'createdAt',
                            title: 'Created Date',
                            sortable: true,
...
export default PurchaseAccFirmInfoInfoTable;
